...
func set_tile_terrain_type_queue(ts: TileSet, td: TileData, type: int) -> void:
	
	var td_meta = _get_tile_meta(td)
	td_meta.type = type
	if type == TileCategory.NON_TERRAIN:
		td_meta = null
	td.set_meta(TERRAIN_META, td_meta)

func set_tile_terrain_type_flush(ts: TileSet) -> void:
...
func set_tile_peering_types_bulk(ts: TileSet, td: TileData, peerings: Array, types: Array) -> bool:
	
	var ts_meta := _get_terrain_meta(ts)
	var td_meta := _get_tile_meta(td)
	
	for peering: int in peerings:
		td_meta[peering] = types
	_set_tile_meta(ts, td, td_meta)
	_purge_cache(ts)
	return true
