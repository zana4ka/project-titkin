...
func handle_primary_attack_input(in_event: InputEvent) -> void:
	...
	if in_event.is_pressed():
		asc.try_activate_abilities_by_tag(TitkinTags.weapon_use_ability, ItemData_Weapon.primary_attack_mode)
	else:
		asc.try_end_abilities_by_tag(TitkinTags.weapon_use_ability)

func handle_secondary_attack_input(in_event: InputEvent) -> void:
	
	if in_event.is_pressed():
		asc.try_activate_abilities_by_tag(TitkinTags.weapon_use_ability, ItemData_Weapon.secondary_attack_mode)
	else:
		asc.try_end_abilities_by_tag(TitkinTags.weapon_use_ability)
...
func handle_special_attack_input(in_event: InputEvent) -> void:
	...
	if in_event.is_pressed():
		asc.try_activate_abilities_by_tag(TitkinTags.weapon_use_ability, ItemData_Weapon.special_attack_mode)
	else:
		asc.try_end_abilities_by_tag(TitkinTags.weapon_use_ability)
